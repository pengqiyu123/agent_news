...
wechat.set_original_author(author=?) ← 已开启原创后修改原创声明里的作者；以输入框右侧 counter_text 为准，不本地估算
...
wechat.inspect_publish_dialog 只读：识别发表确认/无通知发表/继续发表/二维码/账号授权错误
...
13. POST /api/operations/batch                                  一次填完+设置
   {
     "steps": [
       ...
       {"op":"wechat.set_original_author","params":{"author":"AI新闻"}},
